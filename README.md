markdown_content = """# 🎮 逃離職場 RPG (Escape the Workplace)
...
* **數據視覺化 (Data Visualization):** 整合 Plotly 與 Pandas，將生硬的 CSV 業務數據轉化為精美的互動式圖表，完美融入遊戲情境。
* **2D 網頁遊戲引擎:** 使用 HTML5 Canvas 與 Vanilla JavaScript 打造順暢的網格移動系統、碰撞偵測與事件觸發機制。
* **雲端部署:** 完整部署於 Render 雲端平台，並實作環境變數 (Environment Variables) 以確保 API 金鑰安全。
